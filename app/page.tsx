...
const dailyNews = [
  { date:"2026.07.14", sourceName:"Maersk", tag:"市場動態", title:"拉丁美洲港口維持穩定，亞洲出口旺季延續", body:"Maersk 7月市場更新顯示，中美洲、加勒比海主要港口運作大致正常；亞洲至南美東岸的季節性貨量仍支撐穩定班次。", impact:"亞洲旺季貨流維持 → 班次需求穩定 → 訂艙時仍需預留旺季艙位與轉運緩衝。", quiz:"亞洲出口旺季延續時，客戶最適合採取哪個行動？", quizAnswer:"提早訂艙並預留轉運緩衝。", href:"https://www.maersk.com/news/articles/2026/07/14/latin-america-market-update-july" },
  { date:"2026.07.13", sourceName:"IMO", tag:"關鍵水道", title:"IMO 重申國際航道的通行權與安全標準", body:"IMO理事會重申，沿岸國對重要航道採取的交通管理措施，應符合《海上人命安全公約》等國際規範。", impact:"航道管制變動 → 船期與保險不確定性上升 → 航商需要更快調整路線與客戶通知。", quiz:"重要航道管制首先會提高哪類不確定性？", quizAnswer:"船期、路線與保險成本。", href:"https://www.imo.org/en/mediacentre/pressbriefings/pages/imo-council-reaffirms-commitment-to-protecting-vital-shipping-lanes.aspx" },
  { date:"2026.07.13", sourceName:"IMO", tag:"黑海安全", title:"黑海與亞速海商船攻擊事件再起", body:"IMO譴責近期針對民用商船的攻擊，並呼籲各方保障船員、船舶與國際航運安全。", impact:"區域安全惡化 → 繞航、戰爭險與港口風險增加 → 交期與報價需加入更大安全係數。", quiz:"商船遇襲風險升高，報價最可能加入什麼？", quizAnswer:"戰爭險、繞航與交期安全係數。", href:"https://www.imo.org/en/mediacentre/pressbriefings/pages/statement-on-attacks-in-black-sea-and-sea-of-azov.aspx" },
  { date:"2026.07.13", sourceName:"Le Monde", tag:"荷姆茲海峽", title:"荷姆茲海峽通行量再受政策與安全風險擠壓", body:"航跡資料顯示海峽通行船數偏低；新的封鎖與航運收費措施使油價與區域物流風險再度升溫。", impact:"海峽風險升高 → 油價、保險與燃油成本上升 → 運價與附加費面臨調整壓力。", quiz:"荷姆茲海峽風險升高，最直接影響何者？", quizAnswer:"油價、保險、燃油附加費與船期。", href:"https://www.lemonde.fr/en/international/article/2026/07/13/trump-imposes-maritime-toll-in-strait-of-hormuz-as-ship-traffic-collapses_6755446_4.html" },
  { date:"2026.07.01", sourceName:"IMO", tag:"自動航運", title:"首套全球自主船舶安全準則開始適用", body:"IMO的非強制性MASS Code於7月1日生效，為自主貨船的操作、安全與監管建立共同框架。", impact:"自動化標準成形 → 船舶資料與遠端操作需求增加 → 航運服務競爭延伸至科技與風險管理。", quiz:"MASS Code的主要目的為何？", quizAnswer:"建立自主船舶的共同安全與操作框架。", href:"https://www.imo.org/en/mediacentre/pressbriefings/pages/imo-adopts-mass-code.aspx" },
];
// DAILY_NEWS_END
...
export default function Home() {
  ...
  return <main>
    ...
    {active === "news" && <ContentPage no="05" kicker="NEWSROOM" title="時事雷達：現在發生什麼，五年來改變了什麼" desc="上半部追蹤近期決策訊號；下半部建立近五年航運事件的因果脈絡。">
      ...
      <section id="radar" className="chapter"><ChapterTitle eyebrow="LAST CURATED UPDATE · 2026.07.14 08:00 TPE" title="最近五則高面試價值時事" action={complete("radar")}/><div className="article-list">{dailyNews.map((n,i)=><article key={n.title} className={i===0?"featured":""}><div className="article-meta"><time>{n.date}</time><span>{n.tag}</span><em>{n.sourceName}</em></div><h3>{n.title}</h3><p>{n.body}</p><div className="impact"><b>IMPACT CHAIN</b>{n.impact}</div><details><summary>可能怎麼出題？</summary><p><strong>{n.quiz}</strong><br/>答案：{n.quizAnswer}</p></details>{source(n.href,"閱讀原文")}</article>)}</div></section>
